...
final class ViewController: NSViewController, NSDraggingDestination {
        ...
        case .dragging(let valid):
          ...
          if valid {
            ...
            self.mainLabel.stringValue =
              "Please provide a export.xml or export.zip file to continue."
  ...
  func displayInvalidFileTypeAlert() {
    ...
    alert.messageText =
      "Please provide a valid Health.app XML export in either ZIP or XML format to use this utility."
...
extension ViewController: DragViewDelegate {
  ...
  func processZipFile(url: URL) {
    ...
    DispatchQueue.global(qos: .default).async {
      ...
      SSZipArchive.unzipFile(atPath: zip, toDestination: directoryPath, progressHandler: nil) {
        path, succeeded, error in
        var exportItem: URL?
        let enumerator1 = manager.enumerator(
          at: directoryUrl.appendingPathComponent("apple_health_export"),
          includingPropertiesForKeys: [.isDirectoryKey], options: [], errorHandler: nil)
        let enumerator2 = manager.enumerator(
          at: directoryUrl, includingPropertiesForKeys: [.isDirectoryKey], options: [],
          errorHandler: nil)
  ...
  func displayOpenPanel(canChooseFiles: Bool, _ callback: @escaping (URL?) -> Void) {
...
extension ViewController: ParserDelegate {
  ...
  func parsingDidComplete() {
    ...
    guard let directory = directoryPath,
      let scriptUrl = Bundle.main.url(forResource: "zipItUp", withExtension: "sh")
    else {
      return
    }
    ...
    do {
      ...
      DispatchQueue.main.async {
        ...
        alert.addButton(withTitle: "OK")
        alert.messageText =
          "Your health export has been split successfully! Click 'OK' to open the split files in Finder."
        alert.alertStyle = .informational
      ...
      DispatchQueue.main.async {
        ...
        alert.addButton(withTitle: "OK")
        alert.messageText =
          "An error was encountered while completing the splitting process (\(error.localizedDescription)). Please try again."
